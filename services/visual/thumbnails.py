...
macro, before/after split, extreme scale contrast, bold-text tease). Each
concept carries the full click-design brief — title overlay, emotion, color
strategy, focal subject, eye direction, composition — and is scored 0-100 on
seven dimensions (curiosity, readability, contrast, facial focus, object
focus, color, emotion), blended into one overall score and mapped to a
**click probability %**. Deterministic, so results are reproducible and the
Learning Engine can recalibrate against real CTR data by editing weights,
never scoring code.
"""
...
THUMBNAIL_ARCHETYPES = [
    {
        ...
        "focal_subject": "eyes and raised eyebrows, dead center",
        "eye_direction": "direct eye contact with the viewer — impossible to scroll past unseen",
        "emotion": "shock",
        "color_strategy": "warm skin tones against a single saturated background color",
    ...
    },
    {
        ...
        "focal_subject": "the object's most recognizable edge emerging from darkness",
        "eye_direction": "no face — the shadow gradient pulls the eye into the concealed half",
        "emotion": "mystery",
        "color_strategy": "low-key black with one rim-lit accent color",
    ...
    },
    {
        ...
        "focal_subject": "the seam between the two states",
        "eye_direction": "gaze bounces left-to-right across the split — the comparison demands resolution",
        "emotion": "transformation",
        "color_strategy": "desaturated cool left half, vivid warm right half",
    ...
    },
    {
        ...
        "focal_subject": "the size gap between the reference and the subject",
        "eye_direction": "eye travels the diagonal from the tiny reference up to the towering subject",
        "emotion": "awe",
        "color_strategy": "deep atmospheric backdrop with a bright, small focal reference",
    ...
    },
    {
        ...
        "focal_subject": "the missing word implied by the cropped text",
        "eye_direction": "reads the giant type first, then hunts the blurred image for the answer",
        "emotion": "curiosity",
        "color_strategy": "high-contrast white/yellow type on a dark defocused photo",
...
# Click probability calibration: overall 0-100 → realistic 1.5-14% short-form CTR.
CTR_FLOOR_PCT = 1.5
CTR_CEILING_PCT = 14.0


def expected_ctr_pct(overall: int) -> float:
    """Map an overall thumbnail score to a click probability %."""
    return round(CTR_FLOOR_PCT + (CTR_CEILING_PCT - CTR_FLOOR_PCT) * (overall / 100), 1)


# Canonical name for the Cinematic Director era; the old name stays exported.
click_probability_pct = expected_ctr_pct
...
def build_thumbnail_concepts(idea: dict, *, palette: str = "") -> "list[ThumbnailConcept]":
    ...
    for archetype in THUMBNAIL_ARCHETYPES:
        ...
        ctr = expected_ctr_pct(overall)
        title = _title_overlay(idea, archetype)
        color_strategy = archetype["color_strategy"] + (f"; brand palette: {palette}" if palette else "")
        concepts.append(
            ThumbnailConcept(
                ...
                focal_subject=archetype["focal_subject"],
                eye_direction=archetype["eye_direction"],
                ...
                overall=overall,
                click_probability_pct=ctr,
                # Back-compat mirrors (consumed by the UI and audio engine).
                focal_point=archetype["focal_subject"],
                text_overlay=title,
                color_scheme=color_strategy,
                expected_ctr_pct=ctr,
            )
        )
