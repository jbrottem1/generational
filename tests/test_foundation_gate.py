...
def _good_production(**overrides):
    prod = {
...
def test_foundation_gate_passes_good_export():
    gate = evaluate_foundation_export(_good_production())
    assert gate.passed
...
def test_foundation_gate_hard_fails_animation_qc():
    gate = evaluate_foundation_export(_good_production(qc=_good_qc(passed=False)))
    assert not gate.passed
    assert "animation_qc_failed" in gate.hard_fails


def test_foundation_score_hard_fails_low_lipsync():
    # Force low lipsync via mouth_varies=False
    prod = _good_production(qc=_good_qc(mouth_varies=False, passed=True, speaking_ratio=0.05))
...
def test_foundation_gate_flags_equation_outside_write_beat():
